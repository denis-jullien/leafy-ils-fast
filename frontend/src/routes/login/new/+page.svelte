<script lang="ts">
	...
	const { form, dirty, enrichment, formValidity, formValues, isFormValid, submitValues, touched, validity, initialValues } = formula({
...
<div class="flex flex-col gap-6 rounded-box bg-base-200 p-6 max-w-md text-center grow">
	...
	<form class="flex flex-col gap-4" use:form id='signup' action="/auth/reset-password" on:submit|preventDefault={onSubmit}>
...
<!--		[DOM] Password forms should have (optionally hidden) username fields for accessibility: (More info: https://goo.gl/9p2vKq)-->
		...
		<label class="form-control">
			...
			<input class="input input-bordered" type='password' id='password' name='password' autocomplete="new-password" required minlength='8' class:input-error={passwordErr}>
			<div class="label" class:invisible={!passwordErr}>
				<span class="label-text-alt">{$validity?.password?.message}</span>
			</div>

			<meter class="progress " value={$enrichment?.password?.passwordStrength || 0} min='0' max='100' low='33' high='66'
						 optimum='80'></meter>
		</label>
		...
		<label class="form-control">
			...
			<input class="input input-bordered" type='password' id='passwordMatch' name='passwordMatch' autocomplete="new-password" required minlength='8'
